...
dict_repr_lock_held(PyObject *self)
{
    ...
    while (_PyDict_Next((PyObject *)mp, &i, &key, &value, NULL)) {
        ...
        if (res < 0)
            goto error;
        ending_frozenset = PyFrozenSet_CheckExact(value);
    ...
    if (ending_frozenset && _PyUnicodeWriter_WriteChar(&writer, ' ') < 0)
        goto error;
    if (_PyUnicodeWriter_WriteChar(&writer, '}') < 0)
        goto error;
